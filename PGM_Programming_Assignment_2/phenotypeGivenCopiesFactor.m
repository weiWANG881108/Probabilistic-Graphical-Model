...
phenotypeFactor.card = ones(1,3) .* [2, numAlleles, numAlleles];
...
for i=1:length(phenotypeFactor.val)
	if A(i,1) == 1,
		phenotypeFactor.val(i) = alphaList(allelesToGenotypes(A(i,2), A(i,3)));
	else
		phenotypeFactor.val(i) = 1- alphaList(allelesToGenotypes(A(i,2), A(i,3)));
	end;
end;
