...
for k=1:length(genotypeFactor.val);
	parent1 = genotypesToAlleles( A(k,2), : );
	parent2 = genotypesToAlleles( A(k,3), : );
	child = genotypesToAlleles( A(k,1), : );
	for i=1:2
		for j=1:2
			combi = sort([parent1(i), parent2(j)]);
			if isequal(child, combi)
