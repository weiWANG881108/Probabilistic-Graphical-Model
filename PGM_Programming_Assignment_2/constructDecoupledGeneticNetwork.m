...
for (i=1:numPeople)
	if(pedigree.parents(i,1)==0 && pedigree.parents(i,2)==0)
		...
		factorList(i) = childCopyGivenParentalsFactor(numAlleles, i, pedigree.parents(i,1), pedigree.parents(i,2));
		factorList(i+numPeople) = childCopyGivenParentalsFactor(numAlleles, i+numPeople, pedigree.parents(i,1)+numPeople, pedigree.parents(i,2)+numPeople);
	end;

	factorList(i+2*numPeople) = phenotypeGivenCopiesFactor(alphaList, numAlleles, i+2*numPeople, i, i+numPeople);
end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  