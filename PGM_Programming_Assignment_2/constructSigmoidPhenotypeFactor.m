...
for (i=1:length(phenotypeFactor.val))
	...
	for (j=1:length(alleleWeights{1}) )
		value += alleleWeights{1}(j) * (sum( [A(i,2)==j, A(i,4)==j] ));
	end;
	for(j=1:length(alleleWeights{2}))
		value += alleleWeights{2}(j) * (sum( [A(i,3)==j, A(i,5)==j] ));
	end;
	if (A(i,1) == 1)
		phenotypeFactor.val(i) = exp(value)/(1+exp(value));
	else
		phenotypeFactor.val(i) = 1- exp(value)/(1+exp(value));
	end;
